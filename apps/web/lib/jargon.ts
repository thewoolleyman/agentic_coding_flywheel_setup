...
export const jargonDictionary: Record<string, JargonTerm> = {
  ...
  ssh: {
    ...
    why: "SSH is how you'll control your cloud server. Without SSH, you'd have no way to interact with a computer that's physically in a building hundreds of miles away. It's been the industry standard for over 25 years because it just works: it's simple, secure, and supported everywhere. When the wizard asks you to 'SSH into your server,' it's asking you to use this secure connection to start typing commands on your new cloud server.",
  ...
  llm: {
    ...
    why: "AI coding agents are powered by LLMs. Understanding this helps set realistic expectations: LLMs excel at pattern recognition and language tasks, but they can sometimes generate plausible-sounding wrong answers because they're predicting what looks right, not recalling facts from memory. Verification and review of AI output remains important.",
  ...
  postgresql: {
    ...
    long: "PostgreSQL (often 'Postgres') is a reliable, open-source relational database. It stores data in tables with rows and columns, supporting complex queries, transactions, and data integrity. It's the database of choice for many production applications. We install PostgreSQL 18, the latest version with enhanced performance and features.",
